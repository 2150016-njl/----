              ...
              ros::Publisher                             &rt_pub)
{
  ...
  ego_vehicle_info.length = 6.0;  // 显式赋值长
  ego_vehicle_info.width  = 3.0;  // 显式赋值宽
  ...
  for (const auto& obj : objs_msg->objects)
  {
    ...
    // // 保护逻辑：防止感知发来的尺寸为0，导致后续激光雷达仿真射线直接穿透
    // // 如果尺寸太小，强制赋予默认轿车尺寸 (长4.5m, 宽1.8m)
    // if (len < 0.1) len = 4.5; 
    // if (wid < 0.1) wid = 1.8;
...
int main(int argc, char* argv[]) {
  ...
  ros::Publisher rt_pub = nh.advertise<rl_planning::VehicleInfoBatch>("/rt_Obj_info_batch", 2);
  ...
  sync.registerCallback(boost::bind(Callback, _1, _2, boost::ref(pub), boost::ref(rt_pub)));
