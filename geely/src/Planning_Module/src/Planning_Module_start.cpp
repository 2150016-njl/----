...
void get_Module_input(Trajectory_Planning::ExtU_Planning_Module_T &Model_input, 
                    const rl_planning::VehicleInfoBatch::ConstPtr& msg){

    Model_input.X_Vehicle = msg->vehicle_info_batch[0].actor_pos.x;
    Model_input.Y_Vehicle = msg->vehicle_info_batch[0].actor_pos.y;
    // ROS_INFO("X_Vehicle:%0.3f\n",  Model_input.X_Vehicle);

}

void Planning_Module_Callback(const rl_planning::VehicleInfoBatch::ConstPtr& msg){
        // ROS_INFO("fx:%0.3f\n",4.00); 
...
int main(int argc,char** argv)
{
  ...
  ros::Subscriber Planning_Module_start = n.subscribe("vehicle_info_batch", 10, Planning_Module_Callback);
  ros::spin();
  return 0;
}
