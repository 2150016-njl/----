...
class VideoRecorder:
    ...
    def record(self, env):
        if self.enabled:
            if self.cfg.task == 'muti_scenario-v0':
                frame = env.render(mode="topdown",
                                   screen_record=False,
                                   window=False,
                                   show_plan_traj=True,
                                   )
            elif self.cfg.task == 'straight_config_traffic-v0':
                frame = env.render(mode="topdown",
                                   screen_record=False,
                                   window=False,
                                   scaling=6,
    ...
    def __init__(self, log_dir, cfg):
        ...
        if run_offline:
            ...
            try:
                ...
                self._wandb = None
        self._video = VideoRecorder(log_dir, self._wandb, cfg) if self._wandb and cfg.save_video else None
