...
def calc_cur_s(csp, ego_pose):
    """计算车辆当前位置在参考路径上的投影s值（弧长）.

    Args:
        csp: 参考路径（包含s、x、y等信息）
        ego_pose: 车辆当前位姿（x, y, yaw等）
        index: 起始搜索索引（优化性能用）
    ...
    for i in range(start_idx, end_idx):
        x_ref, y_ref = calc_position(csp, csp.s[i])
        dist = np.hypot(ego_pose.x - x_ref, ego_pose.y - y_ref)
    ...
    # === 4. 二次局部精确搜索 ===
    # 在初步找到的最近点附近二次搜索（前后各5个点）
    search_margin = 5
    refined_start = max(0, min_index - search_margin)
    refined_end = min(len(csp.s), min_index + search_margin + 1)

    for i in range(refined_start, refined_end):
        x_ref, y_ref = calc_position(csp, csp.s[i])
        dist = np.hypot(ego_pose.x - x_ref, ego_pose.y - y_ref)
        if dist < min_dist:
            min_dist = dist
    ...
    # 计算匹配点的航向角（处理端点情况）
    if min_index >= len(csp.s) - 1:
        # 如果是最后一个点，用前一点的航向
        yaw_match = calc_yaw(csp, csp.s[min_index - 1])
    else:
        # 否则用当前点的航向
    ...
    # 投影方向单位向量
    direction = np.array([np.cos(yaw_match), np.sin(yaw_match)])
    delta_s = np.dot([delta_x, delta_y], direction)

    # === 7. 最终s值计算与限制 ===
    s = s_match + delta_s
    s = np.clip(s, csp.s[0], csp.s[-1])  # 限制在路径范围内
    s = np.round(s, 2)  # 保留2位小数
...
class preview_point(object):
    def __init__(self, x_vehicle, y_vehicle, phi_vehicle, preview_distance):
        self.x_vehicle = x_vehicle
        self.y_vehicle = y_vehicle
        self.phi_vehicle = phi_vehicle
...
class match_point(object):
    def __init__(self, fpath_x, fpath_y, x_vehicle, y_vehicle):
        self.x_reference_vector = fpath_x
        self.y_reference_vector = fpath_y
        self.x_vehicle = x_vehicle
        self.y_vehicle = y_vehicle
