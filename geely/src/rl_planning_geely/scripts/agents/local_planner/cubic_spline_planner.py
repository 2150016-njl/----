...
class Spline_from_data:
    def __init__(self, nx, a, b, c, d, x, y):
        self.nx = nx
        self.a = a
        self.b = b
        self.c = c
    ...
    def calc(self, t):
        ...
        if t < self.x[0]:
            # print("t < self.x[0]:", t, self.x[0],"result:", self.a[0])
            return self.a[0]
        elif t > self.x[-1]:
            # print("t > self.x[-1]:", t, self.x[-1],"result:", self.a[-1])
            return self.a[-1]
        else:
            # i = max(np.searchsorted(self.x, t) - 1,0)
            # i = np.searchsorted(self.x, t) - 1
            i = np.searchsorted(self.x, t, side='right') - 1
            ...
            result = (self.a[i]
                      ...
                      + self.d[i] * dx ** 3)
            # print("result:", result)
            # print("i:", i, "t:", t,"self.x[i]:", self.x[i]," dx:", dx, "self.a[i]:", self.a[i], " self.b[i]:", self.b[i], " self.c[i]:", self.c[i], " self.d[i]:", self.d[i], " result:", result)
            return result
...
class Spline3D_from_data:
    """
    3D Cubic Spline class
    """

    def __init__(self, s, sx, sy, sz):
        # time1 = time.time()
        self.s = s
        # time2 = time.time()
        self.sx = sx
